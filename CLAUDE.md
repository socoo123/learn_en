...
3. 完整 vocab + quiz；每条生词除中文 `def` 外必有 **`def_en`（英译英）** 与 **`syn`（近义数组）**；文末有难度反馈 UI。
...
用户贴不认识的词 → 补进对应课 `vocab`（必要时 `core`）；未指明则补**最近一课**；不要为此新建一课。新词同样写 `def` + `def_en` + `syn`。
...
| 生词 `def` / `def_en` / `syn` | 中文；英译英；近义数组（1～3） |
